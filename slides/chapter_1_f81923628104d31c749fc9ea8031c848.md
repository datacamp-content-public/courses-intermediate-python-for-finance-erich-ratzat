...
`@script`
We can visually compare both stocks plotting the evolution of accumulated returns in the analyzed period. For this we will use the cumprod() method from pandas. Which returns a DataFrame or Series of the same size containing the cumulative product.
When plotting the cumulative return day after day we will have the following graph:
...
plt.show()
```
![](https://assets.datacamp.com/production/repositories/5053/datasets/1870d6c670117832bd322ce23bd9630bd50efb41/stock_return_comparison.png)


`@script`
A picture is worth a thousand words! We can easily identify that Amazon presented better return than Microsoft! It is possible to see that Amazon had a much higher return during almost all the year but in the last quarter the stock lost breath and the return of both was pretty close on December but on the last days of the year Amazon was a little higher than Microsoft.
